...
def transform(file, tmp_transforms, n):
...
class hic_normalize:
    ...
    def __call__(self, data):
        # matrix = data['hic'].astype(np.float16)
        matrix = data['hic']
...
class transforms:
    ...
    def __call__(self, data) -> Any:
        channels = []
        for transform_type, tmp_transform in self.transform_list:
            print(tmp_transform)
            # transformed_data = tmp_transform(data)
            # if transformed_data.dtype != np.float32:  # 转换为float32
            #     transformed_data = transformed_data.astype(np.float16)
            # channels.append(transformed_data)

            if transform_type in ['01TAD']:
                channels.append(tmp_transform(data, transform_type))
            else:
                channels.append(tmp_transform(data))
...
if __name__ == '__main__':
    ...
    for id, tr in enumerate(trs):
        if tr == 'HiC':
            t.append(('HiC',hic_normalize(cutoff=cutoff)))
        if tr == 'OE':
            t.append(('OE',oe_normalize()))
        if tr == '01TAD':
            t.append(('01TAD',TAD()))
        if tr == 'TADagg':
            t.append(('TADagg', TADaggregate()))
        if tr == 'IS':
            t.append(('IS',insulation_score(distance_upper_bound = 2*bound, normalize=('ISN' in trs))))
        if tr == 'ISN':
            if 'IS' in tr:
                assert trs[id-1] == 'IS', print('ISN should be the next of IS')
            else:
                t.append(('IS',insulation_score(distance_upper_bound = 2*bound, original=False)))
        if tr == 'Lr':
            assert trs[id-1] == 'Lp', print('Lr should be the next of Lp')
            t.append(('Lr', HiCCUPS(distance_upper_bound = 2*bound)))
