...
class HiCCUPS:
    ...
    def __call__(self, data):
        ...
        qvalues_path = os.path.join(self.memmap_dir, 'qvalues.dat')
        ratios_path = os.path.join(self.memmap_dir, 'ratios.dat')
        ratios_shape = (5 + 1, l, l)  # +1 for the extra_ratio expansion
        qvalues_shape = (4, l, l)
        ratios = np.memmap(ratios_path, dtype='float64', mode='w+', shape=ratios_shape)
        qvalues = np.memmap(qvalues_path, dtype='float64', mode='w+', shape=qvalues_shape)
    ...
    #   ratios = np.zeros_like(qvalues).astype(float)
        ...
        ratios[len(kernels), :, :] = extra_ratio
        ...
        out_path = os.path.join(self.memmap_dir, 'out.dat')

        # Calculate the shape based on your specific requirements
        out_shape = (max_qvalue.shape[0] + min_ratio.shape[0], max_qvalue.shape[1], max_qvalue.shape[2])

        # Initialize the memmap for 'out'
        out = np.memmap(out_path, dtype='float64', mode='w+', shape=out_shape)

        # Directly write to 'out' memmap, avoiding the memory-intensive concatenate operation
        out[:max_qvalue.shape[0], :, :] = max_qvalue
        out[max_qvalue.shape[0]:, :, :] = min_ratio

        # Flush changes to ensure all data is written to disk
        out.flush()

        # Since 'out' is a memmap array, it can be returned and used as if it were a regular ndarray,
        # but without requiring all data to be loaded into memory at once.
        return out
