...
def data_divider_parser():
    ...
    req_args.add_argument('-c', default='K562', dest='cell_line',
...
def carn_divider(n, 
    ...
    # Compacting
    hic = compactM(hic, compact_idx, fn='hic.data')
    down_hic = compactM(down_hic, compact_idx, fn='down_hic.data')
...
if __name__ == '__main__':
    ...
    for n in tqdm(chr_list):
        if n in abandon_chromosome:
            continue
        high_file = os.path.join(root_dir, multichannel_matrix_dir, '_'.join(trs), cell_line, f'chr{n}_{high_res}.npz')
        down_file = os.path.join(root_dir, multichannel_matrix_dir, '_'.join(trs), cell_line, f'chr{n}_{low_res}.npz')
